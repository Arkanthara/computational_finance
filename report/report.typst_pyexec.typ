...
- Picking again the same initial times, compute the execution prices you got with this method and compare them with the ones previously obtained.
  ...
  def price_evolution_based(
      ...
      for i in range(random_index, len(dates)):
          if slices_executed >= num_slices:
              ...
              last_price = prices[i]
      assert (
          len(execution_prices) == num_slices
      ), f"Not enough price drops to execute all slices (actual: {len(execution_prices)})... Try reducing delta."
  ...
  for i in range(20):
      ...
      price_evolution_prices = (
          price_evolution_based(dates, asks, random_indices[i]) * 12e6 / 12
      )
      price_evolution_execution_price = np.sum(
          price_evolution_prices
      )  # Assuming equal slices
  ...
  plt.figure()
  plt.bar(
      np.arange(1, 21) - bar_shift,
      differences_price_evolution,
      label="Price Evolution Based Difference",
      width=bar_width,
  )
  plt.bar(
      np.arange(1, 21),
  ...
  #figure(image(".typst_pyexec/figures/cell_7_1.svg"), caption: [Execution Prices vs Decision Prices])
...
def compute_volatility(
    dates: np.ndarray, mid_prices: np.ndarray, window_size: int = 30
) -> np.ndarray:
...
for i in range(20):
    ...
    for j in range(12):
        idx = time_indexer(
            dates,
            dates[random_indices[i]] + datetime.timedelta(minutes=j * 15),
            slice_interval=15,
        )
        if len(idx) > 0:
...
#figure(image(".typst_pyexec/figures/cell_8_1.svg"), caption: [Market Volatility for Selected Intervals])
...
- Compute and compare the execution prices obtained using TWAP, VWAP, and price-evolution strategies across both groups. Which strategy performs best in volatile markets?
  ...
  plt.subplot(1, 2, 1)
  plt.bar(
      np.arange(1, 11) - bar_shift,
      differences_price_evolution[low_volatility_indices],
      label="Price Evolution Based Difference",
      width=bar_width,
  )
  plt.bar(
      np.arange(1, 11),
      differences_vwap[low_volatility_indices],
      label="VWAP Difference",
      alpha=0.7,
      width=bar_width,
  )
  plt.bar(
      np.arange(1, 11) + bar_shift,
      differences_twap[low_volatility_indices],
      label="TWAP Difference",
      alpha=0.7,
      width=bar_width,
  )
  ...
  plt.xlabel("Random Index")
  plt.xticks(
      range(1, 11), [f"Index {i + 1}" for i in low_volatility_indices], rotation=45
  )
  plt.ylabel("Price")
  plt.legend()
  plt.subplot(1, 2, 2)
  plt.bar(
      np.arange(1, 11) - bar_shift,
      differences_price_evolution[high_volatility_indices],
      label="Price Evolution Based Difference",
      width=bar_width,
  )
  plt.bar(
      np.arange(1, 11),
      differences_vwap[high_volatility_indices],
      label="VWAP Difference",
      alpha=0.7,
      width=bar_width,
  )
  plt.bar(
      np.arange(1, 11) + bar_shift,
      differences_twap[high_volatility_indices],
      label="TWAP Difference",
      alpha=0.7,
      width=bar_width,
  )
  ...
  plt.xlabel("Random Index")
  plt.xticks(
      range(1, 11), [f"Index {i + 1}" for i in high_volatility_indices], rotation=45
  )
  plt.ylabel("Price")
  plt.legend()
  plt.show()
  ...
  #figure(grid(columns: 2, [#figure(image(".typst_pyexec/figures/cell_9_1_1.svg"), kind: "subfigure", caption: [Execution Prices vs Decision Prices for Low-Volatility Periods]) <fig1-a>], [#figure(image(".typst_pyexec/figures/cell_9_1_2.svg"), kind: "subfigure", caption: [Execution Prices vs Decision Prices for High-Volatility Periods]) <fig1-b>]), caption: [Execution Prices vs Decision Prices], kind: image) <fig1>
