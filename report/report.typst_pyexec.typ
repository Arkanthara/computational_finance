...
+ Draw a log-log plot of the number of directional changes as a function of the scale $delta$
  for values of $delta in [10^(-5), 10^(-2)]$. What do you observe?
    ...
    #figure(image(".typst_pyexec/figures/cell_8_1.svg"), caption: [Number of directional changes according to the scale $delta$])
...
+ Using $delta = 0.001$ for directional changes:
  ...
  - Plot histograms of both sets of returns. Compare statistical properties (mean, standard
    ...
    #figure(grid(columns: 2, [#figure(image(".typst_pyexec/figures/cell_12_1_1.svg"), kind: "subfigure", caption: [Directional changes returns]) <hist-a>], [#figure(image(".typst_pyexec/figures/cell_12_1_2.svg"), kind: "subfigure", caption: [Daily returns]) <hist-b>]), caption: [Histograms of sets of returns], kind: image) <hist>
